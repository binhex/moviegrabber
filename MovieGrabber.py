...
def _retry(exceptiontocheck, tries=4, delay=3, backoff=2):
...
def config_write(certs_dir_arg, logs_dir_arg, results_dir_arg, webconfig_address_arg, webconfig_port_arg):
        ...
        if certs_dir == "" and certs_dir_arg is None:

                certs_dir = os.path.join(moviegrabber_root_dir, u"certs")

        # if defined via argument then use this
        if certs_dir_arg is not None and os.path.exists(certs_dir_arg):
        ...
        # if not defined via webui and not defined via argument then set to default path
        if logs_dir == "" and logs_dir_arg is None:

                logs_dir = os.path.join(moviegrabber_root_dir, u"logs")

        # if defined via argument then use this
        if logs_dir_arg is not None and os.path.exists(logs_dir_arg):
        ...
        # if not defined via webui and not defined via argument then set to default path
        if results_dir == "" and results_dir_arg is None:

                results_dir = os.path.join(moviegrabber_root_dir, u"db")

        # if defined via argument then use this
        if results_dir_arg is not None and os.path.exists(results_dir_arg):
        ...
        # if not defined via webui and not defined via argument then set to default path
        if webconfig_address == "" and webconfig_address_arg is None:

                webconfig_address = u"0.0.0.0"

        # if defined via argument then use this
        if webconfig_address_arg is not None:
        ...
        # if not defined via webui and not defined via argument then set to default path
        if webconfig_port == "" and webconfig_port_arg is None:

                webconfig_port = u"9191"

        # if defined via argument then use this
        if webconfig_port_arg is not None:
...
def cli_argparse():
        ...
        if uni_to_byte(os.path.exists(os.path.join(moviegrabber_root_dir, u"lib"))):
                ...
                if args["reset_config"] is True and uni_to_byte(os.path.exists(config_ini)):
...
# if defined via argument then use this
if config_dir_arg is not None and os.path.exists(config_dir_arg):
...
class ConfigGeneral(object):
        ...
        def save_config_general(self, **kwargs):
                ...
                config_obj["general"]["queue_max_items_shown"] = kwargs["queue_max_items_shown2"]
                config_obj["general"]["history_max_items_shown"] = kwargs["history_max_items_shown2"]
...
class HistoryRoot(object):
        ...
        def index(self, *args):
                ...
                if history_search_query != ():
                        ...
                        history_max_items_shown = config_obj["general"]["history_max_items_shown"]
...
class QueueRoot(object):
        ...
        def index(self, *args):
                ...
                template.skin_color = config_obj["general"]["skin_color"]
                template.enable_posters = config_obj["switches"]["enable_posters"]
                template.queue_sort_order = config_obj["general"]["queue_sort_order"]
                ...
                if queue_search_query != ():
                        ...
                        queue_sort_order = config_obj["general"]["queue_sort_order"]
                        queue_max_items_shown = config_obj["general"]["queue_max_items_shown"]
        ...
        def queue_sort_order(self, **kwargs):
                ...
                config_obj["general"]["queue_sort_order"] = kwargs["sort_order"]
