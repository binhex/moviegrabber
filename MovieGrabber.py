...
completed
---------
add in option to force download of movies with special cut (extended/dir's/uncut)
bug chrome font boosting on homepage
bug reversed order of regex for groupname from filename
bug starting browser doesnt wait for cherrypy to start
bug imdb bad/good title not working, was due to case issue
bug couldnt remove path from textbox2 if pp rule set to delete
...
current issues
--------------
bug with current version and remote version in footer
...
remove group filtering switch - if preferred/bad group defined assumed enabled
move enable email notification switch  - move to notification/email
move enable post processing switch - move to pp
move enable poster images - move to general/webui
add in sanity check for file size in pp before deleting destination folder
check certification for pp working
...
future
------
add in "Save All" button for usenet/torrent sections to save all settings
add in piratebay api support
consolidate index sections and re-work into methods
fix/test spotweb section
        ...
        def kat_index(self):
                ...
                try:
                        ...
                        try:
                                ...
                                rss_feed = urllib2_retry(rss_feed,user_agent_moviegrabber)
                        ...
                        #use bs to parse rss feed
                        rss_feed_soup = BeautifulSoup(rss_feed)

                        #create list of items in rss feed
                        rss_result_xml = rss_feed_soup.findAll("item")

                        #if rss feed is empty capture error message
                        if not rss_result_xml:

                                rss_error_search = rss_feed_soup.title.string
                        ...
                        for node in rss_result_xml:
                                ...
                                if post_size_search != None:
                                        ...
                                        post_size = post_size_search.contents[0]
                                ...
                                post_details_search = node.find("guid")
                ...
                if torrent_index_site != "":
                        ...
                        for torrent_index_site_item in torrent_index_site_list:
                                ...
                                if config_index_enabled == "yes":
                                        ...
                                        #if index site is torrent site kat, possibly will add additional torrent indexers
